...
# Load data and movies list from corresponding JSON files. [r+ means read mode] and  f is the object.
with open(r'data.json', 'r+', encoding='utf-8') as f:
    data = json.load(f)
with open(r'titles.json', 'r+', encoding='utf-8') as f:
    movie_titles = json.load(f)
    
# Create a method named knn
def knn(test_point, k):
    # Create dummy target variable(which are always n-1) for the KNN Classifier
    ...
    # Print list of 10 recommendations < Change value of k for a different number > Created this table
    table = list()
...
if __name__ == '__main__':
    #we have taken a variable named - genres and stored all the genres in it
    genres = ['Action', 'Adventure', 'Animation', 'Biography', 'Comedy', 'Crime', 'Documentary', 'Drama', 'Family',
              'Fantasy', 'Film-Noir', 'Game-Show', 'History', 'Horror', 'Music', 'Musical', 'Mystery', 'News',
              'Reality-TV', 'Romance', 'Sci-Fi', 'Short', 'Sport', 'Thriller', 'War', 'Western']

    movies = [title[0] for title in movie_titles]
    # Set the Title/header of the page on Streamlit 
    st.header('Movie Recommendation System by - Saiprasad Chavan(A00290598)') 
